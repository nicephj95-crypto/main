...
export function lookupFreightFare(input: {
  ...
  if (!Number.isFinite(km) || km <= 0) {
    return { amount: null, provider: null, rangeLabel: null, vehicleKey: null, status: "NO_DISTANCE", message: "거리 정보가 없어 운송료를 계산할 수 없습니다." };
  }
  ...
  if (group === "MOTORCYCLE" || group === "DAMAS" || group === "LABO") {
    ...
    if (!range) return { amount: null, provider: "INSUNG", rangeLabel: null, vehicleKey, status: "NO_RANGE", message: "해당 거리 구간의 운송료표가 없습니다." };
    const amount = range.rates[vehicleKey];
    return { amount, provider: "INSUNG", rangeLabel: range.distanceLabel, vehicleKey, status: amount == null ? "NO_RATE" : "OK", message: amount == null ? "운송료 데이터 없음: 수동 확인 필요" : null };
  }

  if (group === "ONE_TON_PLUS") {
    const vehicleKey = mapCall24VehicleKey(input.vehicleTonnage);
    if (!vehicleKey) return { amount: null, provider: "CALL24", rangeLabel: null, vehicleKey: null, status: "NO_VEHICLE_RATE_TABLE", message: "해당 차량 운송료표 없음: 수동 확인 필요" };
    const range = findRange(CALL24_FARE_RANGES, km);
    if (!range) return { amount: null, provider: "CALL24", rangeLabel: null, vehicleKey, status: "NO_RANGE", message: "해당 거리 구간의 운송료표가 없습니다." };
    const amount = range.rates[vehicleKey];
    return { amount, provider: "CALL24", rangeLabel: range.distanceLabel, vehicleKey, status: amount == null ? "NO_RATE" : "OK", message: amount == null ? "운송료 데이터 없음: 수동 확인 필요" : null };
  }

  return { amount: null, provider: null, rangeLabel: null, vehicleKey: null, status: "NO_VEHICLE_RATE_TABLE", message: "해당 차량 운송료표 없음: 수동 확인 필요" };
}
